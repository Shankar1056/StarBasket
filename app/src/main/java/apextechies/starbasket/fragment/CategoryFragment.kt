...
class CategoryFragment(private  val sub_cat_id: String, private val id: String) : Fragment(), ProductListAdapter.OnItemClickListener {
    ...
    override fun onQuantityUpdate(item: ProductDataModel, pos: Int) {

        retrofitDataProvider!!.addUpdaDteCart("1", item.id, item.quantity, item.name, item.unitdetails!![pos].selling_price,"1", item.unitdetails[pos].id, object : DownlodableCallback<CartModel> {
    ...
    private fun getCartItem() {
        ...
        retrofitDataProvider!!.cartItem("1", object : DownlodableCallback<CartModel> {
            override fun onSuccess(result: CartModel?) {
                for (i in 0 until result!!.data!!.size) {
                    var adapter = CartAdapter(object : OnCartListener {
                        override fun onCartUpdate(item: CartDataModel?) {

                        }
                    })!!.addItem(result.data!![i])
                    if (productDataModel != null) {
                        productDataModel.quantity = (result.data[i].quantity!!)
                        mAdapter!!.updateViews(i)
                    }
