...
class ProductListAdapter(private val mListener: OnItemClickListener) : RecyclerView.Adapter<ProductListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       ...
       // for (i in 0 until item.unitdetails!!.size) {
       ...
       /* if (!item.unitdetails[0].discount!!.isEmpty()) {
        ...
        if (item.cartdetails!!.size>0) {
			holder.decTV.setVisibility(View.VISIBLE);
			holder.quantityTV.setVisibility(View.VISIBLE)
           /* for (i in 0 until item.cartdetails!!.size) {
                if (item.unitdetails[i].id.equals())
            }*/
			holder.quantityTV.setText(item.cartdetails!![item.selectedIndes!!]!!.quantity!!)
		} else {
        ...
        override fun onClick(v: View) {
            when (v.id) {
                ...
                R.id.tv_dec_quantity -> {
                    ...
                    mListener.onQuantityUpdate(item, item.selectedIndes!!)
                }
